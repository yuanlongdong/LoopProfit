...
# 可选第4参数：超时秒数（默认 1200）
GITHUB_TOKEN=... ./scripts/trigger_android_release.sh <owner> <repo> <tag> 1800
```

该脚本会：自动补 tag（若不存在）→ 触发 `android-apk.yml` → 等待成功 → 输出 APK 下载链接。  
并内置“run 未出现超时”和“整体超时”保护，避免无限等待。
注意：环境变量赋值和脚本路径之间必须有空格，例如 `GH_TOKEN=xxx ./scripts/trigger_android_release.sh ...`。
